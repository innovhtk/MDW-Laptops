...
namespace MDW
{
    ...
    public delegate void CardPresented(string reader, string uid, byte[] cardData);

    public class ReaderList : IDisposable, IEnumerable<string>
    {
        public static ConcurrentDictionary<string, bool> ConcurrentReaders = new ConcurrentDictionary<string, bool>();
        public ReaderList()
        {
            ConcurrentReaders.TryAdd("OMNIKEY CardMan 5x21-CL 0",false);
            ConcurrentReaders.TryAdd("OMNIKEY CardMan 5x21-CL 1",false);
        }
                        ...
                        for (int i = 0; i < states.Length; i++)
                            if ((states[i].dwEventState & SCARD.STATE_CHANGED) != 0)
                                if (i == 0)
                                {
                                    ...
                                    if (tmp == null) Console.WriteLine("No se encontraron lectores HID");
                                    else
                                    {
                                        foreach (var item in tmp)
                                        {
                                            ConcurrentReaders.TryUpdate(item, true, false);
                                        }
                                    }
                                ...
                                else
                                {
                                    ...
                                    if ((states[i].dwEventState & SCARD.STATE_PRESENT) != 0)
                                    {
                                        ...
                                        if (thisCard != lastCard)
                                        {
                                            ...
                                            string uid = "";
                                            //GetUID(ref uid, hContext);
                                            //SelectDevice();
                                            int number = 0;
                                            try
                                            {
                                                number = Convert.ToInt32(states[i].szReader.Substring(states[i].szReader.Length - 1));
                                            }
                                            catch (Exception) { }
                                            SelectDevice(number);
                                            establishContext();
                                            if (connectCard())
                                            {
                                                ...
                                                uid = cardUID; //displaying on text block
                                            }
                                            if (CardPresented != null)
                                                CardPresented(states[i].szReader, uid, cardData);
                                        }
                                    }
                                    else
        ...
        public void SelectDevice(int deviceNumber)
        {
            List<string> availableReaders = this.ListReaders();
            this.RdrState = new Card.SCARD_READERSTATE();
            readername = availableReaders[deviceNumber].ToString();//selecting first device
            this.RdrState.RdrName = readername;
        }
        public void SelectDevice(string readername)
        {
            this.RdrState = new Card.SCARD_READERSTATE();
            this.RdrState.RdrName = readername;
        }
